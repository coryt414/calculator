...
buttons.forEach((button) => {
    button.addEventListener("click", () => {
        if (button.parentElement.className === "numbers-container") {
            ...
            if (displayInput2.textContent !== ''){
                inputNumber2 = Number(displayInput2.textContent)
                expression.push(inputNumber2);
                displayOperator.textContent = operator;
                displayInput2.textContent = '';
                inputNumber1 = operate(expression[0],expression[1],expression[2]);
                expression = [operator, inputNumber1];
                displayInput1.textContent = inputNumber1
            } 
                else {
                inputNumber1 = Number(displayInput1.textContent);
                displayOperator.textContent = operator;
                expression = [operator, inputNumber1]
                return expression
                }
